...
export const Invoice = () => {
  ...
  const invoiceList = useAppSelector(state => state.suuku_webfront.invoice.entities);
  const loading = useAppSelector(state => state.suuku_webfront.invoice.loading);
  const totalItems = useAppSelector(state => state.suuku_webfront.invoice.totalItems);
  ...
  return (
    <div>
      <h2 id="invoice-heading" data-cy="InvoiceHeading">
        <Translate contentKey="suukuWebfrontApp.invoice.home.title">Invoices</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="suukuWebfrontApp.invoice.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/invoice/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="suukuWebfrontApp.invoice.home.createLabel">Create new Invoice</Translate>
          </Link>
        </div>
      </h2>
        ...
        {invoiceList && invoiceList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="suukuWebfrontApp.invoice.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="suukuWebfrontApp.invoice.home.notFound">No Invoices found</Translate>
            </div>
          )
        )}
      </div>
