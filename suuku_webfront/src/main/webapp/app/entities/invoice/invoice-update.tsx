...
export const InvoiceUpdate = () => {
  ...
  const loading = useAppSelector(state => state.suuku_webfront.invoice.loading);
  const updating = useAppSelector(state => state.suuku_webfront.invoice.updating);
  const updateSuccess = useAppSelector(state => state.suuku_webfront.invoice.updateSuccess);
  ...
  return (
    <div>
      <Row className="justify-content-center">
        <Col md="8">
          <h2 id="suukuWebfrontApp.invoice.home.createOrEditLabel" data-cy="InvoiceCreateUpdateHeading">
            <Translate contentKey="suukuWebfrontApp.invoice.home.createOrEditLabel">Create or edit a Invoice</Translate>
          </h2>
        </Col>
      </Row>
      <Row className="justify-content-center">
        <Col md="8">
          {loading ? (
            ...
            <ValidatedForm defaultValues={defaultValues()} onSubmit={saveEntity}>
              ...
              <ValidatedField
                label={translate('suukuWebfrontApp.invoice.code')}
                id="invoice-code"
                name="code"
                data-cy="code"
                type="text"
              ...
              />
              <ValidatedField
                label={translate('suukuWebfrontApp.invoice.date')}
                id="invoice-date"
                name="date"
                data-cy="date"
                type="datetime-local"
              ...
              />
              <ValidatedField
                label={translate('suukuWebfrontApp.invoice.details')}
                id="invoice-details"
                name="details"
                data-cy="details"
                type="text"
              />
              <ValidatedField
                label={translate('suukuWebfrontApp.invoice.status')}
                id="invoice-status"
                name="status"
                data-cy="status"
                type="select"
              >
                {invoiceStatusValues.map(invoiceStatus => (
                  <option value={invoiceStatus} key={invoiceStatus}>
                    {translate(`suukuWebfrontApp.InvoiceStatus.${invoiceStatus}`)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
                label={translate('suukuWebfrontApp.invoice.paymentMethod')}
              ...
              >
                {paymentMethodValues.map(paymentMethod => (
                  <option value={paymentMethod} key={paymentMethod}>
                    {translate(`suukuWebfrontApp.PaymentMethod.${paymentMethod}`)}
                  </option>
                ))}
              </ValidatedField>
              <ValidatedField
                label={translate('suukuWebfrontApp.invoice.paymentDate')}
              ...
              />
              <ValidatedField
                label={translate('suukuWebfrontApp.invoice.paymentAmount')}
