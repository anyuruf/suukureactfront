...
export const Shipment = () => {
  ...
  const shipmentList = useAppSelector(state => state.suuku_webfront.shipment.entities);
  const loading = useAppSelector(state => state.suuku_webfront.shipment.loading);
  const totalItems = useAppSelector(state => state.suuku_webfront.shipment.totalItems);
  ...
  return (
    <div>
      <h2 id="shipment-heading" data-cy="ShipmentHeading">
        <Translate contentKey="suukuWebfrontApp.shipment.home.title">Shipments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="suukuWebfrontApp.shipment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/shipment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="suukuWebfrontApp.shipment.home.createLabel">Create new Shipment</Translate>
          </Link>
        </div>
      </h2>
        ...
        {shipmentList && shipmentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="suukuWebfrontApp.shipment.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('trackingCode')}>
                  <Translate contentKey="suukuWebfrontApp.shipment.trackingCode">Tracking Code</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('trackingCode')} />
                </th>
                <th className="hand" onClick={sort('date')}>
                  <Translate contentKey="suukuWebfrontApp.shipment.date">Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('date')} />
                </th>
                <th className="hand" onClick={sort('details')}>
                  <Translate contentKey="suukuWebfrontApp.shipment.details">Details</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('details')} />
                </th>
                <th>
                  <Translate contentKey="suukuWebfrontApp.shipment.invoice">Invoice</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="suukuWebfrontApp.shipment.home.notFound">No Shipments found</Translate>
            </div>
          )
        )}
      </div>
